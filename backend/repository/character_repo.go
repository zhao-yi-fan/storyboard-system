...
func (r *CharacterRepository) FindByProjectID(projectID int64) ([]models.Character, error) {
	query := `SELECT id, project_id, name, description, avatar_url, created_at, updated_at 
...
func (r *CharacterRepository) FindByID(id int64) (*models.Character, error) {
	...
	var c models.Character
	err := database.DB.QueryRow(query, id).Scan(&c.ID, &c.ProjectID, &c.Name, &c.Description, &c.AvatarURL, &c.CreatedAt, &c.UpdatedAt)
...
func (r *CharacterRepository) Create(c *models.Character) error {
	query := `INSERT INTO characters (project_id, name, description, avatar_url) VALUES (?, ?, ?, ?)`
	
	result, err := database.DB.Exec(query, c.ProjectID, c.Name, c.Description, c.AvatarURL)
	if err != nil {
		return err
	}
...
func (r *CharacterRepository) Update(c *models.Character) error {
	query := `UPDATE characters SET name = ?, description = ?, avatar_url = ? WHERE id = ?`
	
	_, err := database.DB.Exec(query, c.Name, c.Description, c.AvatarURL, c.ID)
	return err
}
