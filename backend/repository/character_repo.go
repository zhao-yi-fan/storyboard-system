...
func (r *CharacterRepository) FindByProjectID(projectID int64) ([]models.Character, error) {
	query := `SELECT id, project_id, name, description, avatar_url, avatar_preview_url, design_sheet_url, design_sheet_preview_url, created_at, updated_at
	...
	for rows.Next() {
		...
		if err := rows.Scan(&c.ID, &c.ProjectID, &c.Name, &c.Description, &avatarURL, &avatarPreviewURL, &designSheetURL, &designSheetPreviewURL, &c.CreatedAt, &c.UpdatedAt); err != nil {
			return nil, err
		}
...
func (r *CharacterRepository) FindByID(id int64) (*models.Character, error) {
	...
	var designSheetURL sql.NullString
	var designSheetPreviewURL sql.NullString
	err := database.DB.QueryRow(query, id).Scan(&c.ID, &c.ProjectID, &c.Name, &c.Description, &avatarURL, &avatarPreviewURL, &designSheetURL, &designSheetPreviewURL, &c.CreatedAt, &c.UpdatedAt)
...
func (r *CharacterRepository) Create(c *models.Character) error {
	query := `INSERT INTO characters (project_id, name, description, avatar_url, avatar_preview_url, design_sheet_url, design_sheet_preview_url) VALUES (?, ?, ?, ?, ?, ?, ?)`

	result, err := database.DB.Exec(query, c.ProjectID, c.Name, c.Description, c.AvatarURL, c.AvatarPreviewURL, c.DesignSheetURL, c.DesignSheetPreviewURL)
	if err != nil {
		return err
	}
...
func (r *CharacterRepository) Update(c *models.Character) error {
	query := `UPDATE characters SET name = ?, description = ?, avatar_url = ?, avatar_preview_url = ?, design_sheet_url = ?, design_sheet_preview_url = ? WHERE id = ?`

	_, err := database.DB.Exec(query, c.Name, c.Description, c.AvatarURL, c.AvatarPreviewURL, c.DesignSheetURL, c.DesignSheetPreviewURL, c.ID)
	return err
}
