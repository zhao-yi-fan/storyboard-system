...
func (s *StoryboardCoverService) selectReferenceImages(storyboard *models.Storyboard, scene *models.Scene, publicBaseURL string) ([]StoryboardCoverReferenceImage, []string, error) {
	...
	for _, asset := range assets {
		...
		if url := s.resolveReferenceURL(publicBaseURL, strings.TrimSpace(asset.CoverURL)); url != "" {
		...
		if url := s.resolveReferenceURL(publicBaseURL, strings.TrimSpace(asset.FileURL)); url != "" {
...
func (s *StoryboardCoverService) resolveReferenceURL(publicBaseURL, rawURL string) string {
	trimmed := strings.TrimSpace(rawURL)
	if trimmed == "" {
		return ""
	}
	if s.ossService.IsEnabled() && IsGeneratedAssetPath(trimmed) {
		return s.ossService.ResolveGeneratedURL(trimmed)
	}
	return absolutizeReferenceURL(publicBaseURL, trimmed)
}
...
func (s *StoryboardCoverService) downloadAndStore(ctx context.Context, storyboardID int64, sourceURL string) (string, string, error) {
	...
	publicPath := GeneratedPublicPath("covers", filename)

	if s.ossService.IsEnabled() {
		tmp, err := os.CreateTemp("", "storyboard-cover-*"+filepath.Ext(filename))
		if err != nil {
		...
		if err := s.ossService.EnsureUploaded(tmp.Name(), publicPath); err != nil {
			return "", "", fmt.Errorf("上传封面到 OSS 失败: %w", err)
		}
