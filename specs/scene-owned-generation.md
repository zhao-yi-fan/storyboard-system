...
4. 参考区展示后端本次真实会传入模型的完整图片集合；每张图可预览并标明名称、类型和来源。
5. 保存片段 Prompt，设置时长、分辨率和音频。
6. 点击“生成首帧”或“重新生成首帧”，确认 Prompt、参考图映射和模型后开始生成。
7. 预览并生成一次片段封面或片段视频。
8. 在片段级历史中切换或删除版本。
...
- 参考态：当前首帧作为生成结果单独展示；真实生成参考显示完整数量、名称、类型、来源及缺失项。
- 错误态：参考加载、生成预览和正式生成失败时，在当前片段内保留错误信息，可重试或关闭。
...
- `GET /api/scenes/:id/generation-references`
...
- 片段缺少 Prompt 时拒绝生成。
- Prompt 中有 `@名称` 但未绑定、或已绑定图片未在 Prompt 中引用时，预览返回明确映射状态。
- 生成参考区、确认弹窗和正式模型请求必须使用同一组图片 URL 与顺序。
- 首帧预览或生成失败时不得只写浏览器控制台，必须在当前片段保留可见错误。
...
- 项目总片继续按片段顺序合成。
- “生成首帧”只生成当前片段的一张视频首帧；已有首帧时按钮显示“重新生成首帧”。
- 当前首帧不冒充本次首帧生成的参考图；如果视频生成使用当前首帧，应在视频设置区单独说明。
- 参考图缩略图与后端实际请求字段一致：角色使用主设定图，资产使用封面或原图。
- 所有真实参考图均可点击查看原图，超过可见卡片数量时显示剩余数量并可展开。
- Prompt 编辑区能看到已绑定参考及其是否被 `@名称` 引用；最终 Prompt 预览包含按传入顺序生成的参考图语义映射。
- 只有一个可用画面模型时显示静态模型信息，不展示无效下拉选择器。
