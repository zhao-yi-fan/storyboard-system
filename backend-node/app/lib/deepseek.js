...
function buildUserPrompt(scriptText) {
...
async function parseScriptWithDeepSeek(config, scriptText) {
  ...
  const resp = await fetch(`${config.deepSeekBaseUrl.replace(/\/$/, '')}/chat/completions`, {
    method: 'POST',
    headers: {
  ...
  const normalized = extractJSONObject(
    content.replace(/^```json/, '').replace(/^```/, '').replace(/```$/, '').trim()
  );
