...
class MediaGenerationService extends Service {
  ...
  async markCurrent(storyboardId, mediaType, generationId) {
    ...
    try {
      ...
      await conn.execute(
        'UPDATE storyboard_media_generations SET is_current = 0 WHERE storyboard_id = ? AND media_type = ?',
        [storyboardId, mediaType],
      );
      await conn.execute(
        'UPDATE storyboard_media_generations SET is_current = 1 WHERE id = ? AND deleted_at IS NULL',
        [generationId],
      );
      await conn.commit();
    } catch (err) {
    ...
    await this.pool.execute(
      ...
      [
        ...
        Object.prototype.hasOwnProperty.call(payload, 'status')
          ...
          : current.status,
        Object.prototype.hasOwnProperty.call(payload, 'result_url')
          ? payload.result_url
            ? String(payload.result_url)
            : null
          : current.result_url || null,
        Object.prototype.hasOwnProperty.call(payload, 'preview_url')
          ? payload.preview_url
            ? String(payload.preview_url)
            : null
          : current.preview_url || null,
        Object.prototype.hasOwnProperty.call(payload, 'source_url')
          ? payload.source_url
        ...
        Object.prototype.hasOwnProperty.call(payload, 'is_current')
          ? payload.is_current
            ? 1
            : 0
          : current.is_current
