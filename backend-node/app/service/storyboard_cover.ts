...
class StoryboardCoverService extends Service {
  ...
  async previewCoverGeneration(id, selectedModel) {
    ...
    const { references, missing } = await this.ctx.service.storyboardReference.selectReferenceImages(storyboard, scene);
  ...
  async uploadCover(id, thumbnailUrl) {
    ...
    await this.ctx.service.mediaGeneration.markCurrent(id, MEDIA_TYPE.COVER, generation.id);
    const nextStoryboard = await this.ctx.service.storyboard.applyMediaGeneration(id, generation);
    return {
