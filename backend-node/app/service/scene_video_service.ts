...
class SceneVideoService extends Service {
  async previewVideoGeneration(
    ...
    const boundReferences = [
      ...baseReferences,
      ...this.ctx.service.sceneCover.buildVideoFrameReferences(scene.video_frame_references || []),
    ];
  ...
  async composeVideo(id: number, regenerate: boolean) {
    ...
    await this.ctx.service.scene.update(id, { video_status: GENERATION_STATUS.GENERATING, video_error: '' });
    try {
      const filename = `${sanitizeFileName(`scene-${id}`)}-${Date.now()}.mp4`;
      ...
      const sceneWithPoster = await this.ctx.service.scene.update(id, { video_poster_url: posterUrl });
