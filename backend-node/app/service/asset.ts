...
class AssetService extends Service {
  ...
  async update(id, payload) {
    ...
    await this.pool.execute(
      `UPDATE assets
       ...
       WHERE id = ?`,
      [
        Object.prototype.hasOwnProperty.call(payload, 'character_id')
          ? characterId
          : (current.character_id ?? null),
        Object.prototype.hasOwnProperty.call(payload, 'name')
          ? String(payload.name || '').trim() || current.name
          : current.name,
        Object.prototype.hasOwnProperty.call(payload, 'type')
          ? String(payload.type || '').trim() || current.type
          : current.type,
        nextFile,
        nextFile !== normalizeGeneratedAssetReference(this.app, current.file_url)
          ? ''
  ...
  async softDelete(id) {
    ...
    if (Number(rows[0]?.count || 0) > 0) {
      throw new Error(
        `该资产已被 ${Number(rows[0].count)} 个镜头引用，暂不允许删除。请先让相关镜头改用其他资产或重新生成封面。`,
      );
    }
  ...
  async ensurePreview(asset) {
    ...
    const preview = await createPreviewFromSource(
      this.app,
      source,
      'assets',
      `asset-${asset.id}`,
    ...
    await this.pool.execute('UPDATE assets SET thumbnail_url = ? WHERE id = ?', [
      preview,
      asset.id,
    ]);
    asset.thumbnail_url = resolveUrl(
      this.app,
      preview,
      this.app.config.storyboard.publicAppBaseUrl || '',
    );
