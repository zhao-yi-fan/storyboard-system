...
describe('AI client configuration', () => {
  it('groups provider defaults by configuration category', () => {
    ...
    assert.equal(AI_REQUEST_TIMEOUT.SEEDREAM_SECONDS, DEFAULT_REQUEST_TIMEOUT_SECONDS.SEEDREAM);
    assert.equal(AI_REQUEST_TIMEOUT.WANX_VIDEO_SECONDS, DEFAULT_REQUEST_TIMEOUT_SECONDS.LONG);
    assert.equal(AI_REQUEST_TIMEOUT.VOICE_SECONDS, DEFAULT_REQUEST_TIMEOUT_SECONDS.VOICE);
  });
